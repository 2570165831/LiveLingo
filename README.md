...
- **当前应用与打包源码**对应固定候选 6（`0.1.0 / 20260922.2`）。在本轮文档收尾前，冻结清单 2233 个文件逐项 SHA-256 一致；更新本 README 后重新核对，清单内唯一差异为 `README.md`，应用、测试与打包源码没有漂移。整改包括课程状态恢复、持久化转写队列、复查意见汇集、资源归属检查、导入收尾与学习笔记来源约束等。
...
macOS 14 最终候选真实运行仍未完成。当前 Parallels 登记的 macOS 14 VM 位于外置卷 `光樞雷獄`，该物理设备目前没有出现在系统磁盘列表中，因此 VM 状态为 `invalid` 且资源不可达。本项目不会把静态最低系统版本检查或旧 VM 结果改写成这次 macOS 14 实测通过。当前面向 Apple 芯片；Intel 与 Universal 构建未验证。
