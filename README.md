...
- **当前发布源码**的内部构建号为 `20260923.1`。应用功能源码与 20260922 已验收候选一致；本轮重新编译，并把此前被忽略的测试清单加入仓库，使全新克隆也能运行相应测试。整改包括课程状态恢复、持久化转写队列、复查意见汇集、资源归属检查、导入收尾与学习笔记来源约束等。
- **本机已安装应用**仍为 `0.1.0 / 20260922.2`；它的状态不代表新安装包已替换本机应用。
- **新离线交付件**为 `LiveLingo-0.1.0-20260924.0014-macOS14+-arm64.dmg`。它在 9 月 23 日由当前源码重新构建，完整内置离线模型与运行库；公开标签按 9 月 24 日 00:14（悉尼时间）收到的公证通过结果命名。App 内部构建号仍为 `20260923.1`。签名、公证、包内核验及本次发布边界见 [20260924 发布说明](RELEASE_NOTES_20260924.md)。20260922 安装包的公开下载仍保持撤回。
